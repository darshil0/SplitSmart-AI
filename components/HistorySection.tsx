...
const HistorySection: React.FC<HistorySectionProps> = ({ history, onDelete, onClearAll }) => {
  if (history.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-slate-50/50 rounded-3xl border border-slate-200 border-dashed">
        ...
        <p className="text-slate-400 text-xs mt-1 max-w-[200px]">Your saved splits will appear here for future reference.</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm animate-in fade-in duration-500">
      <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
        <div>
          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{history.length} Saved Entries</p>
        </div>
        <button 
          onClick={onClearAll}
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-1">
              {entry.participants.slice(0, 3).map((p, idx) => (
                <span key={idx} className="text-[9px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded-md font-medium">
                  {p}
                </span>
              ))}
