      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 scroll-smooth">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center px-8">
            ...
            <p className="text-slate-900 font-bold text-sm">Ready to help split!</p>
            <p className="text-slate-400 text-xs mt-1">Try: "John had the burger"</p>
          </div>
        )}
        ...
        {messages.map((msg) => (
          ...
          >
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 border shadow-sm ${
              msg.role === 'user' ? 'bg-white border-slate-200 text-slate-600' : 'bg-indigo-600 border-indigo-500 text-white'
            }`}>
      ...
      <form onSubmit={handleSubmit} className="p-4 bg-white border-t border-slate-100">
        <div className="relative group">
          <input
            ...
            placeholder={disabled ? "Waiting for receipt..." : "e.g., Tom had the salad"}
